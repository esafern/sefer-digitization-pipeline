...
1. **Klal 179 word 16**: `יותה` $\rightarrow$ `יותר` (*"...מוסיף ומדקדק בהבטה יותה מהרואה..."* — 3,326× attested; clear $\text{ה/ר}$ OCR error).
2. **Klal 30 words 250 & 1263**: `גכי` $\rightarrow$ `גבי` (*"...גכי יש יד לפאה..."* and *"...אבל גכי עדות..."* — 4,780× attested; clear $\text{כ/ב}$ OCR error).
3. **Klal 54 word 730**: `עלירם` $\rightarrow$ `עליהם` (*"...ולא דבר עלירם מטוב ועד רע..."* — 2,169× attested; clear $\text{ר/ה}$ OCR error).
4. **Klal 7 word 252**: `הלכרה` $\rightarrow$ `הלכה` (*"...ותמהני על עומק הלכרה של מהר"ש..."* — 2,290× attested; stray $\text{ר}$ insertion).
5. **Klal 168 word 162**: `אאביי` $\rightarrow$ `אביי` (*"...אאביי קי"ל כאביי..."* — 2,973× attested; duplicate $\text{א}$ insertion).
6. **Klal 30 word 1115**: `טיניה` $\rightarrow$ `מיניה` (*"...לאסתיועי טיניה ע"ש ודון מינה..."* — 2,722× attested; $\text{ט/מ}$ OCR error).
7. **Klal 24 words 166 & 230**: `ואידן` $\rightarrow$ `ואידך` (*"...איכא למעט לכש ואידן מכללא..."* — final-nun $\text{ן}$ misread for final-kaf $\text{ך}$).
...
1. **Proper Nouns**: Klal 30 word 1007: `זלמן` $\rightarrow$ `זמן` (*"...כה"רר משה שלמה זלמן ני'..."* — the detector stripped $\text{ל}$ from the name "Zalman").
2. **Halachic Technical Terms**: Klal 2 word 188 & Klal 11 word 81: `בשרש` $\rightarrow$ `בשר` (*"...בס' המצות בשרש השני..."* — the detector stripped $\text{ש}$ from "Shoresh" [root/principle] to yield "meat").
3. **Literary Vocabulary**: Klal 123 word 24: `תוהה` $\rightarrow$ `תורה` (*"...ואני תוהה מאד..."* — "wondering/astonished" proposed to become "Torah").
4. **Inflected Forms**: Klal 169 word 982: `במקורם` $\rightarrow$ `במקום` (*"...הרואה הדברים במקורם..."* — "in their source" proposed to become "in place of").
5. **Aramaic Verbs**: Klal 167 word 1062: `אכחד` $\rightarrow$ `אחד` (*"...לא אכחד תחת לשוני..."* — "I shall not conceal" proposed to become "one").
...
* **12 flags (5.9%) — Already Fixed in Corpus (Resolved Ghost Flags)**:
  * The current corpus text already contains the proposed reading (e.g. Klal 4 w95 is already `"כתבו"`, Klal 88 w510 is already `"אבל"`, Klal 88 w533 is `"וכתבו"`, Klal 2 w30 is `"בפסחים"`).
...
* **1 flag (0.5%) — Drifted Index**:
  * **Klal 43 word 14**: Currently points to `"ומחתינן"`. The note targets `"ממטונא"` $\rightarrow$ `"מממונא"`, which sits at **word index 17** (shifted due to a 3-word insertion/geresh merge earlier in that klal).
* **1 flag (0.5%) — Out of Scope / Missing**:
  * Klal 16 word 162 target word was rewritten during truncation repair.
...
|---|---|---|
| `local-backfill-2026-08-17` | 62 | Retroactive word-level anchors for multi-word phrases and disputed markers. |
| `ai-lexicon-gap-part1-v2` | 11 | Rare forms with zero lexicon attestation (e.g. `דנראח` w229 in klal 1, `בתלמור` w802 in klal 150). |
| `tools/review_lexicon_gaps.py` | 10 | High-confidence lexicon-only corrupt forms purged on 2026-08-26 (`כרתב` $\rightarrow$ `כתב`, `בסרק` $\rightarrow$ `בפרק`, `שרוא` $\rightarrow$ `שהוא`). |
| `tools/validate_part1_corpus_integrity.py` | 7 | Non-Hebrew character intrusions (`&` for `ﭏ`, `Π` for folio, `!` for dot). |
| `ai-pattern-b-sweep-incidental` | 6 | Part 2 legacy flags (klalim 227, 245, 265) recorded before the Part 2/3 freeze; currently out of bounds for Part 1. |
| `ai-candidate-second-sort` | 4 | Unanimous 3-of-3 engine disagreements (DocAI + Surya + VLM agree against stored text, e.g. klal 38 w286 `דרכה` $\rightarrow$ `דרבה`). |
| `tools/detect_real_word_substitution.py` | 4 | Real word substitution flags (`איידו` $\rightarrow$ `איידי`, `במשרו` $\rightarrow$ `במשהו`). |
| `tools/validate_catchword_continuity.py` | 2 | Seam furniture intrusions (klal 74 w416 `אמר`, klal 210 w66 `לא`). |
| `tools/lookup_sefaria_dictionaries.py` | 1 | Klal 177 w340 `למיפך`: **Explicitly retracted** on 2026-08-26 after dictionary verification proved it valid, but flag record remains open. |
| `local` | 1 | Klal 66 w135: Scan-confirmed ink correction (`ע"ס` $\rightarrow$ `ע"פ`). |
...
3. **Index Stability**: Only 1 of 202 Round 4 flags experienced position drift (Klal 43 w14 $\rightarrow$ w17), confirming that text index stability has remained high across the last two weeks of rebuilds.
