...
0BT. **[2026-09-06] SELF-INFLICTED: `git checkout -- .` DESTROYED NINE FILES OF
    ...
    Logged because it is provenance, not just embarrassment: five production
    files in commits `e88571b`..`1be20dc` were reconstructed from patch text
    rather than restored from a copy, and anyone auditing them should know which.

    ### What happened

    Setting up the commit split, `git checkout -- .` was run to get a clean base.
    The backup taken minutes earlier covered `PROJECT-STATUS.md`,
    `START_HERE.md`, `tests/`, `review_decisions.jsonl`,
    `DRIFTED-RULINGS-WORKLIST.md` and `rebuild_all.sh` - **and no `pipeline/` or
    `tools/` source at all.** Nine tracked files of finished, tested,
    uncommitted work were reverted to HEAD. Untracked files (the four new
    modules) survived, because checkout does not touch them.

    Lesson 44 is the entry about mutating the working tree from underneath live
    work, and this is its shape with nobody else to blame: the destructive step
    was bookkeeping, not the work itself, and the backup was scoped to the files
    that happened to be on the mind at the time rather than to the diff.

    ### What was recoverable, and how

    | file | source |
    |---|---|
    | `apply_reviewer_decisions.py` | `ard2.bak` (mid-session), complete |
    | `export_corpus.py` | `export_corpus.new.py`, complete |
    | `review_server.py` | `rs.bak` + word_id edits re-applied |
    | `patch_witness_word_indices.py` | `pw.bak` + argparse and internal-name fixes re-applied |
    | **`review_decisions.py`** | **rebuilt from transcript** |
    | **`scan_alignment.py`** | **rebuilt from transcript** |
    | **`apply_punctuation_decisions.py`** | **rebuilt from transcript** |
    | **`flag_unreviewed_auto_corrections.py`** | **rebuilt from transcript** |
    | **`list_drifted_rulings.py`** | **rebuilt from transcript** |

    **Verified rather than assumed**: the gate returned to 460 passed;
    `DRIFTED-RULINGS-WORKLIST.md` regenerated BYTE-IDENTICAL to the pre-loss
    copy; `flag_unreviewed_auto_corrections.py` still converges at 0 with the
    idempotence guard intact; the worklist still reports 26 rulings with 11
    settled by shift; all four commits pass the gate independently in their own
    worktrees. No evidence anything was lost. A rebuild-from-transcript is
    nonetheless weaker evidence than a restore, so **the five bold rows are where
    to look hardest.**

    ### Two follow-on errors inside the recovery

    * **A patch with no assertion silently no-matched.** The argparse guard for
      `patch_witness_word_indices` was written against an anchor
      (`_out_path()`) that the restored file did not yet contain (`OUT_PATH`), so
      `str.replace` did nothing and reported success. Every patch after that
      carries an `assert <anchor> in s`.
    * **The witness-queue footgun fired a SECOND time**, while that unguarded
      version was briefly in place, from a `--help` smoke test - nulling the same
      6 `word_index` values against the shifted corpus. Reverted from git again.
      Two trips in one day, both from `--help`, is the argument for the guard
      that now exists.

    ### Rules taken from this

    1. **Never `git checkout -- .`** in this repo. Stage subsets instead; the
       working tree does not need reverting to build a split commit.
    2. **A backup for a commit split must be the DIFF, not a hand-picked file
       list.** `git diff > patch` and `git stash create` both capture everything;
       enumerating files captures what you remembered.
    3. The commits themselves were built with `git hash-object -w` +
       `git update-index --cacheinfo`, which stages arbitrary content **without
       touching the working tree** - that is the safe way to split, and it is
       what the four commits actually used after the loss.
