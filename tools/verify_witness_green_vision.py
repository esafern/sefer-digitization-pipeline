...
# NO green_word_indices(). It called review_server.api_word_states() in process
# to enumerate exactly the words the dashboard paints green - which is the right
# source, and is forbidden: test_no_pipeline_stage_imports_the_review_server
# holds the line that NOTHING outside tests/ imports the live HTTP server
# module, and this tool is in tools/. Caught by that test on the rebuild, not by
# me reading the rule first.
#
# Re-deriving green here instead would be Lesson 13 THE SECOND COPY OF THE
# TRUTH: the tri-state is already encoded twice (review_counts.word_states and
# app.js wordState), pinned across both by
# test_nav_tristate_matches_what_each_word_actually_renders_as, and a third copy
# living in a triage script is how those two came apart in the first place.
#
# So the set is defined by what this tool CAN establish on its own: every live
# witness_choice whose queue row maps it to a word. That is NOT quite the same
# set, and the difference is one row rather than none - measured 2026-09-09
# against /api/word-states, 29 mapped rulings of which 28 render green. The odd
# one out is klal 88 w518, where the witness ruling says `בפ׳`, the corpus holds
# `בס'` and the word renders MACHINE-DISPUTED, not green: a candidate dispute is
# still open there and owns the colour. Including it is the right error to make
# in a triage tool - it is a real live ruling disagreeing with the corpus - but
# the report says "witness rulings placed on a word", not "green words", because
# that is what it actually enumerates.
def collect():
...
def main():
    ...
    print(f"{len(rows)} live witness rulings; {len(todo)} map to a word and have a bbox to crop")
...
def write_report(rows):
    ...
    lines = ["# Witness rulings placed on a word, adjudicated against the ink",
             "",
             ...
             "",
             f"- live witness rulings (mapped to a word, plus unmapped): {len(rows)}",
