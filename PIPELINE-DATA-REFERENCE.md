...
reference for the data shapes, not the process — see `CLAUDE.md`'s
"Pipeline shape" and "Directory layout" sections for the scripts that
...
docai_word_boxes/page_N.json         ← raw OCR tokens, one file per scan page
  ...
  │   CLAUDE.md's "Pipeline shape" — produced the first version of the
...
`review_decisions.jsonl` is never touched by any of it — see CLAUDE.md's
...
failure mode CLAUDE.md's Lesson 13 documents — this file exists precisely
...
being compared (CLAUDE.md's Lesson 12: keying on the crop alone let a
...
both are still live and consulted independently — see CLAUDE.md's Lesson
...
(see CLAUDE.md's "Human review decisions"). Nothing in this file is ever
